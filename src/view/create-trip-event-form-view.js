...
const createEventTypeTemplate = (event) =>
  `<div class="event__type-item">
    <input id="event-type-${event}-1" class="event__type-input  visually-hidden" type="radio" name="event-type" value="${event}">
    <label class="event__type-label  event__type-label--${event}" for="event-type-taxi-1">${capitalizeFirstLetter(event)}</label>
  </div>`;
  ...
  `<div class="event__type-wrapper">
    ...
    <div class="event__type-list">
      <fieldset class="event__type-group">
        ...
        ${EVENTS.map((event) => createEventTypeTemplate(event)).join('')}
      </fieldset>
    </div>
  </div>`;
...
const createAddTripEventFormTemplate = () =>
  `<form class="event event--edit" action="#" method="post">
    ${createEventHeaderTemplate()}
    ${createEventDetailsTemplate()}
  </form>`;

export default class CreateTripEventFormView {
  getTemplate() {
    return createAddTripEventFormTemplate();
  }
